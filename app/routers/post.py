...
def get_posts(db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user), limits: int = 10, skip: int = 0):

    posts = db.query(models.Post).limit(limits).offset(skip).all()
    return posts
